...
collection = db.mention_usa_hillary.find()

with open('statistics.csv', 'wb') as csvfile:
	fieldnames = ['id_Tweet','datetime_creation','date_creation', 'time_creation','latitud','longitud','type_location',
	...
	for document in collection:
		if 'retweeted_status' not in document:
			if 'entities' in document:
				...
				if any(u['screen_name'] == 'HillaryClinton' for u in user_mentions) or any(u['screen_name'] == 'realDonaldTrump' for u in user_mentions):
					...
					if document['user']['location'] != None:
						location=document['user']['location'].encode('utf8')
					...
					writer.writerow({ 'id_Tweet':document['id'],'datetime_creation': datetime_creation,'date_creation': date_creation,
						'time_creation': time_creation,'latitud':lat,'longitud':lon,'type_location':type_location,
